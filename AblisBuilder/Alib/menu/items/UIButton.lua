...
local SettingsButton = {
    ...
    Text = { X = 8, Y = 5, Scale = 0.26 },
    LeftBadge = { Y = 2, Width = 30, Height = 30 },
    RightBadge = { X = 395, Y = -2, Width = 40, Height = 40 },
    RightText = { X = 435, Y = 4, Scale = 0.26 },
        ...
        if CurrentMenu.Pagination.Minimum <= Option and CurrentMenu.Pagination.Maximum >= Option then
            ...
            if Active then
                if Style.Color and Style.Color.HightLightColor then
                    ...
                    RenderRectangle(CurrentMenu.X, CurrentMenu.Y + SettingsButton.SelectedSprite.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, SettingsButton.SelectedSprite.Width + CurrentMenu.WidthOffset, SettingsButton.SelectedSprite.Height, 255, 255, 255, 210)
                end
            end
            if Enabled then
